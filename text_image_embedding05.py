...
class Entry:
    ...
    image_id: str
    metadata: Metadata
    base_name: str = ""  # base_nameフィールドを追加
    ...
    def _create_entries(self, metadata: Metadata, filename: str):
        ...
        for ref_id, context in metadata.figure_contexts.items():
            ...
            entry = Entry(
                ...
                image_id=image_id,
                metadata=metadata,
                base_name=base_name  # base_nameを設定
            )
...
def build_metadata_fields(data: Entry, metadata_info: Metadata, fig_base_id: str) -> Dict:
    ...
    content_id = f"{data.base_name}_content"  # data.text_idからdata.base_nameに変更
...
def build_image_metadata_fields(data: Entry, metadata_info: Metadata, fig_base_id: str) -> Dict:
    """画像のメタデータフィールドを構築"""
    # base_nameを使用してIDを生成
    text_id = f"{data.base_name}_Fig{data.number}_text"
    ...
    image_metadata_fields = {
        ...
        "summary": metadata_info.summary,
        "entry_type": data.type,
        "related_text_id": text_id,  # data.text_idではなくtext_idを使用
        "related_content_id": content_id  # base_nameから生成したcontent_idを使用
    }
